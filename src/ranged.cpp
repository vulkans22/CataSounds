...
void generate_gun_sound( std::string weapon_id ) {
    //auto weapon_id = g->u.weapon.typeId();
    play_sound_effect("fire_gun", weapon_id, 100, 0);
}
                                                         ...
                                                         const tripoint &target_arg, double shot_dispersion )
{
    ...
    int npcpos = g->npc_at( source );
    if(npcpos != -1) {
        npc *p = g->active_npc[npcpos];
        auto weapon_id = p->weapon.typeId();
        generate_gun_sound( weapon_id );
    }

    int monpos = g->mon_at( source );
    if(monpos != -1) {
        auto *monster = g->monster_at( source );
        auto monster_id = monster->type->id;
        if ( monster_id == "mon_turret" || monster_id == "mon_secubot" ) {
            generate_gun_sound( "hk_mp5" );
        } else if ( monster_id == "mon_turret_rifle" || monster_id == "mon_chickenbot" || monster_id == "mon_tankbot") {
        generate_gun_sound( "m4a1" );
        }
    }
    ...
    for( size_t i = 0; i < trajectory.size() && ( dam > 0 || stream ); i++ ) {
        ...
        if (do_animation) {
            if (anim_skip == OPTIONS["PROJECTILE_SKIP"]) {
        ...
        for (unsigned long int i = 0; i < g->num_zombies(); i++) {
            ...
            if( rl_dist( z.pos(), tp ) <= 4 &&
                ...
                if (!z.has_effect("bounced")) {
                    ...
                    generate_gun_sound( "bio_lightning_tail" );
...
void make_gun_sound_effect(player &p, bool burst, item *weapon)
{
    ...
    if( ammo_used == "40mm") {
        if( p.is_player() || p.is_npc() != -1 || p.is_monster() ) {
            sounds::sound(p.pos(), 8, _("Thunk!"), false, "none", "none");
        } else {
            sounds::sound(p.pos(), 8, _("Thunk!"), false, "fire_gun", weapon->typeId());
        }
    } else if( weapon_id == "hk_g80") {
        if( p.is_player() || p.is_npc() != -1 || p.is_monster() ) {
            sounds::sound(p.pos(), 24, _("tz-CRACKck!"), false, "none", "none");
        } else {
        ...
        if( p.is_player() || p.is_npc() != -1 || p.is_monster() ) {
            sounds::sound(p.pos(), 4, _("Fwoosh!"), false, "none", "none");
        } else {
            sounds::sound(p.pos(), 4, _("Fwoosh!"), false, "fire_gun", weapon->typeId());
        }
    } else if( noise > 0 && ammo_used != "pebble" &&
            ammo_used != "fishspear" && ammo_used != "dart" ) {
        if( p.is_player() || p.is_npc() != -1 || p.is_monster() ) {
            sounds::sound(p.pos(), noise, gunsound, false, "none", "none");
        } else {
