...
void play_sound_effect(std::string id, std::string variant, int volume, int loops) {
#ifdef SDL_SOUND
    ...
    if (Mix_PlayChannel(-1, effect_to_play->chunk, loops) == -1) {
...
void play_sound_effect_fade(std::string id, std::string variant, int volume, int channel, int duration) {
#ifdef SDL_SOUND
    sound_effect_entry *effect_to_play = get_sound_effect(id, variant);
    if (!effect_to_play) {
        return;
...
int paused_channel(int channel) {
#ifdef SDL_SOUND
...
void fade_group(int tag, int duration) {
#ifdef SDL_SOUND
