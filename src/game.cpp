...
void game::do_ambient_sfx()
{
    /* Channel assignments:
    502: Daytime outdoors env
    503: Nighttime outdoors env
    504: Underground env
    505: Indoors env
    506: Indoors rain env
    507: Outdoors snow env
    508: Outdoors flurry env
    509: Outdoors thunderstorm env
    510: Outdoors rain env
    511: Outdoors drizzle env
    Group Assignments:
    1: SFX related to weather
    2: SFX related to time of day
    */
    set_group_channels(504, 511, 1);
    set_group_channels(502, 503, 2);
    w_point const w = weatherGen.get_weather( u.global_square_location(), calendar::turn );
    weather = weatherGen.get_weather_conditions(w);

    if ( calendar::turn.is_night() && !night_sfx_playing && !is_sheltered(u.pos())) {
        fade_group(2, 1000);
        play_sound_effect_fade("environment", "nighttime", 100, 503, 1000);
        night_sfx_playing = 1;
        day_sfx_playing = 0;
    } else if ( !calendar::turn.is_night() && !day_sfx_playing && !is_sheltered(u.pos())) {
        fade_group(2, 1000);
        play_sound_effect_fade("environment", "daytime", 100, 502, 1000);
        night_sfx_playing = 0;
        day_sfx_playing = 1;
    }

    if (( is_underground(u.pos()) && !underground_sfx_playing ) || ( is_underground(u.pos()) && weather != prev_weather )) {
        fade_group(1, 1000);
        fade_group(2, 1000);
        indoor_sfx_playing = 0;
        outdoor_sfx_playing = 0;
        night_sfx_playing = 0;
        day_sfx_playing = 0;
        underground_sfx_playing = 1;
        play_sound_effect_fade("environment", "underground", 100, 504, 1000);
    } else if (( is_sheltered(u.pos()) && !is_underground(u.pos()) && !indoor_sfx_playing ) ||
               (is_sheltered(u.pos()) && !is_underground(u.pos()) && weather != prev_weather )) {
        fade_group(1, 1000);
        fade_group(2, 1000);
        play_sound_effect_fade("environment", "indoors", 100, 505, 1000);
        if ( weather == WEATHER_RAINY || weather == WEATHER_DRIZZLE || weather == WEATHER_THUNDER
            || weather == WEATHER_LIGHTNING ) {
            play_sound_effect_fade("environment", "rain_car", 100, 506, 1000);
        }
        indoor_sfx_playing = 1;
        outdoor_sfx_playing = 0;
        night_sfx_playing = 0;
        day_sfx_playing = 0;
        underground_sfx_playing = 0;
    } else if (( !is_sheltered(u.pos()) && !outdoor_sfx_playing ) || ( !is_sheltered(u.pos()) && weather != prev_weather )) {
        fade_group(1, 1000);
        if ( weather == WEATHER_DRIZZLE ) {
            play_sound_effect_fade("environment", "WEATHER_DRIZZLE", 100, 511, 1000);
        } else if ( weather == WEATHER_RAINY ){
            play_sound_effect_fade("environment", "WEATHER_RAINY", 100, 510, 1000);
        } else if ( weather == WEATHER_THUNDER || weather == WEATHER_LIGHTNING ){
            play_sound_effect_fade("environment", "WEATHER_THUNDER", 100, 509, 1000);
        } else if ( weather == WEATHER_FLURRIES ) {
            play_sound_effect_fade("environment", "WEATHER_FLURRIES", 100, 508, 1000);
        } else if ( weather == WEATHER_SNOW || weather == WEATHER_SNOWSTORM ) {
            play_sound_effect_fade("environment", "WEATHER_SNOW", 100, 507, 1000);
        }
    indoor_sfx_playing = 0;
    outdoor_sfx_playing = 1;
    underground_sfx_playing = 0;
    }
    prev_weather = weather;
    set_channel_volume(502, 100);
    set_channel_volume(503, 100);
    set_channel_volume(504, 100);
    set_channel_volume(505, 100);
    set_channel_volume(506, 100);
    set_channel_volume(507, 100);
    set_channel_volume(508, 100);
    set_channel_volume(509, 100);
    set_channel_volume(510, 100);
    set_channel_volume(511, 100);
}
    ...
    if (toSwimmable && toDeepWater && !toBoat) { // Dive into water!
        ...
        if (!u.has_artifact_with(AEP_STEALTH) && !u.has_trait("LEG_TENTACLES") &&
            !u.has_trait("DEBUG_SILENT")) {
                ...
                if (std::chrono::duration_cast<std::chrono::milliseconds>(footstep_time).count() > 250) {
